...
void multiplyCollapse3(const double *mat1, const double *mat2T, double *result) {
...
void multiplyCollapse2(const double *mat1, const double *mat2T, double *result) {
  const int BLOCK_SIZE = 256;
  const int UNROLL = 8; // 8 * double = 512
...
int main() {
  try {
    ...
    multiplyCollapse2(mat1, transposed, result);
    auto t2 = high_resolution_clock::now();
    duration<double, std::milli> ms_double = t2 - t1;

    double collapse2 = ms_double.count();
    std::cout << "\nCollpase 2 Time = " << collapse2 << "ms\n";
    ...
    multiplyCollapse3(mat1, transposed, result);
    t2 = high_resolution_clock::now();
    ms_double = t2 - t1;

    double collapse3 = ms_double.count();
    std::cout << "\nCollapse 3 Time = " << collapse3 << "ms\n";
    std::cout << "Custom Multiplication Performance = " << calculateGFLOPS(ms_double.count())
              << " GFLOPS/s\n";
    writeMatrixToCSV(result, "data/multiThread.csv", R1, C2);
    ...
    writeTimingToCSV("data/time.csv", ms_double.count(), collapse2, collapse3);
