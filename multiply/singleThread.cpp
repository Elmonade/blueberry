...
void mulMatWithUnrolledI(const double *mat1, const double *mat2T, double *result) {
  ...
  for (int i = 0; i < R1; i += UNROLL) {
    for (int j = 0; j < C2; j++) {
      double sum0 = 0, sum1 = 0, sum2 = 0, sum3 = 0;
      double sum4 = 0, sum5 = 0, sum6 = 0, sum7 = 0;

      for (int k = 0; k < C1; k++) {
        double rcM2 = mat2T[j * C1 + k];

        sum0 += mat1[i * C1 + k] * rcM2;
        sum1 += mat1[(i + 1) * C1 + k] * rcM2;
        sum2 += mat1[(i + 2) * C1 + k] * rcM2;
        sum3 += mat1[(i + 3) * C1 + k] * rcM2;
        sum4 += mat1[(i + 4) * C1 + k] * rcM2;
        sum5 += mat1[(i + 5) * C1 + k] * rcM2;
        sum6 += mat1[(i + 6) * C1 + k] * rcM2;
        sum7 += mat1[(i + 7) * C1 + k] * rcM2;
      }
